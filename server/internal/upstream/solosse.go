...
func Aggregate(r io.Reader) (map[string]any, error) {
	...
	if len(toolOrder) > 0 {
		...
		// OpenAI 规范：带 tool_calls 的 assistant 消息 finish_reason 应为 tool_calls。
		// 上游 SOLO 有时在 done 事件里仍报 "stop"，这里统一归一化，兼容严格客户端。
...
func streamOpts(w http.ResponseWriter, r io.Reader, onErr func(*SOLOStreamError)) error {
	...
	for {
		...
		if ev := scanLine(st, strings.TrimRight(line, "\r\n")); ev != nil {
			...
			case "done":
				...
				if sawToolCalls && finish != "tool_calls" {
					// 与 Aggregate 一致：流式下上游 done 常报 "stop"，归一化为 tool_calls
					finish = "tool_calls"
				}
	...
	if !sawDone {
		...
		// 必须补一个 finish_reason chunk：DSH/OpenAI 客户端要求流以
		// finish_reason 结尾，否则判定 "Stream ended without finish_reason"。
